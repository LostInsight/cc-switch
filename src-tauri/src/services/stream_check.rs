...
static PROMPT_ROTATION_CURSOR: AtomicUsize = AtomicUsize::new(0);
static RANDOM_VARIABLE_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\{random\[(-?\d+)-(-?\d+)\]\}").expect("valid random variable regex")
});
...
impl StreamCheckService {
    ...
    pub async fn check_with_retry(
        ...
        let cursor = PROMPT_ROTATION_CURSOR.fetch_add(1, Ordering::Relaxed);
        let prompt_template = Self::select_prompt_template(&effective_config, cursor);
        effective_config.test_prompt = Self::render_prompt_template(&prompt_template);
    ...
    fn merge_provider_config(
        ...
        // Per-provider testConfig was removed in v3.17. Keep the helper so the
        // real-request pipeline remains compatible with the current command API.
        global_config.clone()
    }

    fn select_prompt_template(config: &StreamCheckConfig, cursor: usize) -> String {
        ...
        if !prompts.is_empty() {
            return prompts[cursor % prompts.len()].to_string();
        }

        let legacy_prompt = config.test_prompt.trim();
    ...
    fn render_prompt_template(template: &str) -> String {
        Self::render_prompt_template_for_date(template, chrono::Local::now().date_naive())
    }

    fn render_prompt_template_for_date(template: &str, date: chrono::NaiveDate) -> String {
        let with_date = template.replace("{yyyyMMdd}", &date.format("%Y%m%d").to_string());

        RANDOM_VARIABLE_RE
            .replace_all(&with_date, |captures: &Captures<'_>| {
                let Some(min) = captures[1].parse::<i64>().ok() else {
                    return captures[0].to_string();
                };
                let Some(max) = captures[2].parse::<i64>().ok() else {
                    return captures[0].to_string();
                };
                ...
                let range_size = (i128::from(max) - i128::from(min) + 1) as u128;
                let random = u128::from_be_bytes(*uuid::Uuid::new_v4().as_bytes());
                let value = i128::from(min) + (random % range_size) as i128;
                value.to_string()
            })
            .into_owned()
    }
...
mod tests {
    ...
    #[test]
    fn test_legacy_prompt_config_remains_usable() {
        let config: StreamCheckConfig = serde_json::from_value(serde_json::json!({
            "testPrompt": "Legacy prompt"
        }))
        .unwrap();

        assert!(config.test_prompts.is_empty());
        assert_eq!(
            StreamCheckService::select_prompt_template(&config, 7),
            "Legacy prompt"
        );
    }

    #[test]
    fn test_prompt_templates_rotate_and_skip_empty_values() {
        let config = StreamCheckConfig {
            test_prompt: "Legacy prompt".to_string(),
            test_prompts: vec![
        ...
        assert_eq!(
            StreamCheckService::select_prompt_template(&config, 0),
            "First prompt"
        );
        assert_eq!(
            StreamCheckService::select_prompt_template(&config, 1),
            "Second prompt"
        );
        assert_eq!(
            StreamCheckService::select_prompt_template(&config, 2),
            "First prompt"
        );
    }

    #[test]
    fn test_prompt_variables_render_date_and_inclusive_random_range() {
        let date = chrono::NaiveDate::from_ymd_opt(2026, 7, 19).unwrap();

        for _ in 0..32 {
            let rendered = StreamCheckService::render_prompt_template_for_date(
                "date={yyyyMMdd}; random={random[1-100]}; fixed={random[7-7]}; invalid={random[9-2]}; unknown={name}",
                date,
            );
            let fields: std::collections::HashMap<_, _> = rendered
                .split("; ")
                .filter_map(|part| part.split_once('='))
                .collect();
            let random = fields["random"].parse::<i64>().unwrap();

            assert_eq!(fields["date"], "20260719");
            assert!((1..=100).contains(&random));
            assert_eq!(fields["fixed"], "7");
            assert_eq!(fields["invalid"], "{random[9-2]}");
            assert_eq!(fields["unknown"], "{name}");
        }
    }
    ...
    #[test]
    fn test_grokbuild_resolves_selected_upstream_model() {
